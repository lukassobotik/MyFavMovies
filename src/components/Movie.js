...
export default function Movie({item, index, rowId, type}) {
    ...
    return (<div id={"itemId" + index + "-" + rowId} className='w-[200px] sm:w-[240px] md:w-[280px] inline-block cursor-pointer relative p-2'>
            <div className="row_item" onMouseOver={hover} onMouseLeave={hide} onClick={click}>
                ...
                <div id={"card" + index + "-" + rowId} className="bg-black invisible rounded-b whitespace-nowrap">
                    <div className="text-white whitespace-normal text-xs md:text-sm font-extrabold inline-block w-fit overflow-scroll flex-nowrap justify-center items-center h-full text-center">{item?.title}</div>
